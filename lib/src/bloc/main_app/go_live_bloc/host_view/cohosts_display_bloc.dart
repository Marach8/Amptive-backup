...
class AmptiveGoLiveSelectCoHostBloc
    extends Cubit<List<ObjectWithNotifier<Host>>> {
  AmptiveGoLiveSelectCoHostBloc()
      : super(List.generate(
            maxCoHost, (_) => ObjectWithNotifier(obj: Host.empty())));
  ...
  void hostAddCohost(ObjectWithNotifier<Host> coHost) {
    final List<ObjectWithNotifier<Host>> newState =
        List<ObjectWithNotifier<Host>>.from(state)
            .where((ObjectWithNotifier<Host> member) =>
                (member.obj.profilePicture ?? '').isNotEmpty)
            .toList();
    ...
    if (newState.length < 5 && !newState.contains(coHost)) {
      ...
      newState.addAll(List.generate(
          difference, (_) => ObjectWithNotifier(obj: Host.empty())));
