...
class GetLiveProgramEntryTokenCubit extends Cubit<ATAppState<dynamic>> {
  ...
  Future<void> getLiveProgramEntryToken({required String livestreamId}) async {
    emit(LoadingState<dynamic>(currentData: currentData));
    try {
      final ApiResponse<dynamic> response =
          await goLiveRepo.getLiveProgramEntryToken(livestreamId: livestreamId);
      response.when(
      ...
      emit(FailureState<dynamic>('Unable to get live program entry token: $e',
          oldData: currentData));
    }
  }
}