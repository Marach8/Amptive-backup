...
class CohostServiceBloc
    extends Cubit<(List<ATCohost<bool>>, List<ATCohost<bool>>)> {
  CohostServiceBloc() : super((initialCohosts, initialSelectedCohosts));

  static List<ATCohost<bool>> initialCohosts = getCoHostList();
  static List<ATCohost<bool>> initialSelectedCohosts =
      List<ATCohost<bool>>.filled(5, ATCohost<bool>.empty());

  void addCohost(ATCohost<bool> incomingCohost) {
    final List<ATCohost<bool>> newCoHosts = List<ATCohost<bool>>.from(state.$2);

    final int noOfAddedCoHosts = newCoHosts
        .where((ATCohost<bool> coHost) => coHost.profilePicture != null)
        .length;

    if (noOfAddedCoHosts == 5) return;

    final int firstEmptyIndex = newCoHosts
        .indexWhere((ATCohost<bool> cohost) => cohost.profilePicture == null);
  ...
  void removeCohost(ATCohost<bool> outGoingCohost) {
    final List<ATCohost<bool>> copyOfState =
        List<ATCohost<bool>>.from(state.$2);
    if (!copyOfState.contains(outGoingCohost)) return;

    final List<ATCohost<bool>> newCoHosts = copyOfState
        .where(
  ...
  void searchCoHosts(String searchKey) {
    final List<ATCohost<bool>> filterCoHosts = initialCohosts
        .where((ATCohost<bool> coHost) =>
