...
class CalenderProgramBloc extends Bloc<ProgramsEvents, ProgramsState> {
  CalenderProgramBloc() : super(NoProgramsState()) {
    on<LoadProgramsEvent>(
        (LoadProgramsEvent event, Emitter<ProgramsState> emit) async {
      emit(ProgramsLoadingState());
      try {
        await Future.delayed(const Duration(seconds: 5));
        emit(ProgramsDataState(programs: <String, List<CalenderProgram>>{
