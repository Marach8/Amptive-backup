  ...
  Widget build(BuildContext context) {
    return switch (tabType) {
              ...
              return switch (state) {
                ...
                SuccessState<HostedShowsResponseModel>() => Builder(
                  builder: (_) {
                    ...
                    final List<HostedShow> hostedShows = showsData?.hostedShows ?? <HostedShow>[];
                      ...
                      if (state is FailureState<HostedShowsResponseModel>) {
                        return Center(
                          child: IconButton(
                            ...
                            onPressed: () => context.read<HostedShowsCubit>().fetchHostedShows(),
                          ),
                        );
                      }
                          ...
                          child: Row(
                            children: <Widget>[
                              ClipRRect(
                                ...
                                child: ATImgLoader(
                                  imgPath: show.coverUrl ?? ATImgStrings.weCanDoHardThingsBgImage,
                                  height: 77,
                                  width: 77,
                                ),
                              ),
                                  ...
                                  children: <Widget>[
                                    Row(
                                      children: <Widget>[
                                        ...
                                        Flexible(
                                          child: Text(
                                            show.title ?? 'We Can Do Hard Things',
                                    ...
                                    Text(
                                      ...
                                      style: context.textTheme.bodyMedium?.copyWith(fontSize: ATSizes.size15),
                                    ),
                                    Row(
                                      children: <Widget>[
                                        ATPaidIndicatorIcon(size: 10, radius: 1, color: ATColors.hexC2C2C2),
                                        const SizedBox(width: 5),
                                        Text(
                                          show.category ?? 'Society',
                                          style: context.textTheme.titleSmall?.copyWith(color: ATColors.hexC2C2C2),
                                        ),
                                        const SizedBox(width: 5),
                                        ATCircleAvatar(diameter: 3, color: ATColors.hexC2C2C2),
                                        const SizedBox(width: 5),
                                        Text(
                                          show.createdAt ?? 'Date',
                                          style: context.textTheme.titleSmall?.copyWith(color: ATColors.hexC2C2C2),
      ...
      ProfileTabType.events => BlocProvider<HostedEventsCubit>(
          ...
          child: BlocBuilder<HostedEventsCubit, ATAppState<HostedEventsResponseModel>>(
            builder: (BuildContext context, ATAppState<HostedEventsResponseModel> state) {
              return switch (state) {
                InitialState<HostedEventsResponseModel>() ||
                LoadingState<HostedEventsResponseModel>() ||
                FailureState<HostedEventsResponseModel>() ||
                SuccessState<HostedEventsResponseModel>() => Builder(
                  builder: (_) {
                    final HostedEventsResponseModel? eventsData =
                        context.read<HostedEventsCubit>().currentHostedEventsData;
                    final List<HostedEvent> hostedEvents = eventsData?.hostedEvents ?? <HostedEvent>[];
                    if (hostedEvents.isEmpty) {
                      ...
                      if (state is FailureState<HostedEventsResponseModel>) {
                        return Center(
                          child: IconButton(
                            icon: const Icon(Icons.refresh),
                            onPressed: () => context.read<HostedEventsCubit>().fetchHostedEvents(),
                          ),
                        );
                      }
                      return const Center(child: Text('Nothing to show here yet...'));
                          ...
                          child: Row(
                            children: <Widget>[
                              ClipRRect(
                                borderRadius: BorderRadius.circular(5),
                                child: ATImgLoader(
                                  imgPath: event.coverUrl ?? ATImgStrings.weCanDoHardThingsBgImage,
                                  height: 77,
                                  width: 77,
                                ),
                              ),
                                  ...
                                  children: <Widget>[
                                    ...
                                    Row(
                                      children: <Widget>[
                                        ...
                                        const SizedBox(width: 5),
                                        Text(
                                          event.startedAt?.toNormalDate ?? 'Date',
                                          style: context.textTheme.titleSmall?.copyWith(color: ATColors.hexC2C2C2),
                                        ),
                                      ],
                                    )
              ...
              return switch (state) {
                ...
                SuccessState<HostedEventsResponseModel>() => Builder(
                  builder: (_) {
                    final HostedEventsResponseModel? eventsData =
                        context.read<HostedEventsCubit>().currentHostedEventsData;
                    final List<HostedEvent> allEvents = eventsData?.hostedEvents ?? <HostedEvent>[];
                    final List<HostedEvent> filteredEvents = allEvents.where((HostedEvent event) =>
                        event.status == 'scheduled' ||
                        (event.scheduledFor != null && DateTime.tryParse(event.scheduledFor!)?.isAfter(DateTime.now()) == true)).toList();
              ...
              return switch (state) {
                ...
                SuccessState<HostedEventsResponseModel>() => Builder(
                  builder: (_) {
                    ...
                    final List<HostedEvent> filteredEvents = allEvents.where((HostedEvent event) =>
                        event.status == 'ended' ||
                        (event.endedAt != null && DateTime.tryParse(event.endedAt!)?.isBefore(DateTime.now()) == true)).toList();
                    if (filteredEvents.isEmpty) {
                      if (state is LoadingState<HostedEventsResponseModel>) {
                        return const ProgramDisplayShimmer();
                      }
                                  ...
                                  children: <Widget>[
                                    ...
                                    Row(
                                      children: <Widget>[
                                        ...
                                        const SizedBox(width: 5),
                                        Text(
                                          event.endedAt?.toNormalDate ?? 'Date',
                                          style: context.textTheme.titleSmall?.copyWith(color: ATColors.hexC2C2C2),
                                        ),
                                      ],
                                    )
