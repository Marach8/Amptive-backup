      ...
      child: Builder(builder: (BuildContext ctx) {
        ...
        return BlocListener<_PrivateBloc, List<bool>>(
          ...
          child: Column(
            ...
            children: _list.indexed
                .map(((int, String) item) =>
                    _CustomWidget(text: item.$2, index: item.$1))
                .toList(),
          ),
        );
      }),
...
class _CustomWidget extends StatelessWidget {
  const _CustomWidget({required this.text, required this.index});
  final String text;
  final int index;
    ...
    return BlocSelector<_PrivateBloc, List<bool>, bool>(
        ...
        builder: (_, bool isVisible) {
          return AnimatedOpacity(
            ...
            onEnd: () => isVisible
                ? context.read<_PrivateBloc>().triggerNext(index + 1)
                : null,
            child: Padding(
