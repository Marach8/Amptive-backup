...
class WithdrawalBanksBloc
    extends Bloc<WithdrawalBanksEvents, WithdrawalBanksState> {
  WithdrawalBanksBloc() : super(WithdrawalBanksInitial()) {
    ...
    on<FetchBanksEvent>(
        (FetchBanksEvent event, Emitter<WithdrawalBanksState> emit) async {
      emit(FetchingBanks());
      await Future.delayed(const Duration(seconds: 5));
      emit(WithdrawalBanksData(banks: banks, selectedBank: null));
    });

    on<SearchBanksEvent>(
        (SearchBanksEvent event, Emitter<WithdrawalBanksState> emit) async {
      ...
      final List<String> searchResult = banks
          .where((String bank) =>
              bank.toLowerCase().contains(event.query.toLowerCase()))
          .toList();
      ...
      emit(WithdrawalBanksData(banks: searchResult, selectedBank: null));
    });

    on<SelectBankEvent>(
        (SelectBankEvent event, Emitter<WithdrawalBanksState> emit) async {
      if (state is WithdrawalBanksData) {
        final WithdrawalBanksData currentState = state as WithdrawalBanksData;
        emit(WithdrawalBanksData(
            banks: currentState.banks, selectedBank: event.bank));
      }
    });
...
class WithdrawalBanksData extends WithdrawalBanksState {
  WithdrawalBanksData({required this.banks, required this.selectedBank});
